...
export default function CreatorPage() {
  ...
  const metrics = [
    { label: t.totalPlays, value: dashboard?.totalPlays ?? uploads.reduce((s, tr) => s + tr.playCount, 0) },
    { label: t.totalLikes, value: dashboard?.totalLikes ?? uploads.reduce((s, tr) => s + tr.likeCount, 0) },
    { label: t.totalReposts, value: dashboard?.totalReposts ?? uploads.reduce((s, tr) => s + tr.repostCount, 0) },
    { label: t.totalComments, value: dashboard?.totalComments ?? uploads.reduce((s, tr) => s + tr.commentCount, 0) },
  ];
                    ...
                    <div className="space-y-2">
                      {uploads.map((track) => {
                        ...
                        return (
                          <div
                            key={track.id}
                            className={`rounded-lg p-4 transition-colors border-l-2 ${
                              active
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="space-y-1 min-w-0">
                                  <div className="flex flex-wrap items-center gap-2">
                                    <p className="font-medium text-foreground truncate">{trackCard.title}</p>
                                    <Badge variant={statusTone[track.status ?? "published"] ?? "soft"} className="text-[10px]">
                                      {track.status ?? "published"}
                                    </Badge>
                                  </div>
                                  <p className="text-xs text-muted-foreground">
                                    {trackCard.genreLabel} · {formatDuration(track.duration)} · {formatCompactNumber(track.playCount)} {t.plays}
                                  </p>
                                </div>
                              </div>
                            </button>
                  ...
                  {selectedAnalytics.data && selectedTrackCard && (
                    <div className="rounded-lg bg-muted/30 p-4 space-y-3 border border-border">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium text-foreground truncate">{selectedTrackCard.title}</p>
                        <span className="text-xs text-muted-foreground shrink-0">{t.analytics}</span>
                      </div>
                      <div className="grid grid-cols-4 gap-2 text-center">
                        {[
                          ...
                          <div key={label}>
                            <p className="text-lg font-bold text-foreground">{formatCompactNumber(value)}</p>
                      ...
                      {selectedAnalytics.data.recentListeners.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-foreground mb-2">{t.recentListeners}</p>
                          <div className="space-y-1.5">
                            {selectedAnalytics.data.recentListeners.slice(0, 4).map((listener) => (
                              ...
                              >
                                <span className="font-medium text-foreground">{listener.username}</span>
